...
@Component({
  selector: 'app-user-link-menu',
  templateUrl: './user-link-menu.component.html',
  styleUrls: ['./user-link-menu.component.css']
})
export class UserLinkMenuComponent implements OnInit {
  //所有的结点数据
  nodes: TreeNode[];
  ...
  //保存按钮事件
  save(){
    const url = '/api/user-manager/userlinkmenu/' + this.userInfo.userAccount;
  ...
  //初始化node结点
  initNodes(){
    this.http.get('/api/user-manager/build-all-menu').subscribe(
      (res) => {
        this.nodes = <TreeNode[]>res;
        const url = '/api/user-manager/find-linked-menu/' + this.userInfo.userAccount;
