...
@Component({
  selector: 'app-user-link-menu-site',
  templateUrl: './user-link-menu-site.component.html',
  styleUrls: ['./user-link-menu-site.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class UserLinkMenuSiteComponent implements OnInit {
  //所有的结点数据
  nodes: TreeNode[];
  ...
  //初始化结点
  initNodes(){
    const url = '/api/user-manager/build-linked-menu-site/' + this.userInfo.userAccount;
    this.http.get(url).subscribe(
      (res) => {
        this.nodes = <TreeNode[]>res;
        const url = '/api/user-manager/find-linked-menu-site/' + this.userInfo.userAccount;
          this.http.get(url).subscribe(
          (res) => {
